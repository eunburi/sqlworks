...
--추천수 칼럼을 추가하세요(칼럼명 : cnt, 자료형 : NUMBER DEFALUT 0)
...
--WHERE ROWNUM > 11 AND ROWNUM <= 20; --ROWNUM은 1을 포함해야한다.
...
FROM
    ...
    FROM board)
WHERE rn > 11 AND rn <= 20; -- ROWNNUM의 별칭을 사용하면 가능함
...
SELECT ROWID, bno, title, content
FROM board
WHERE ROWID = 'AAAS1QAAHAAAAFdAAA';
